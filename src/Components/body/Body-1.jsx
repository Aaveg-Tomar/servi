    ...
    <div>
      <Box className="hero" pos={"relative"}>
        ...
        >
          ...
          >
            <VStack gap={5}>
              <Text className="heading">SERVICELY</Text>
              ...
              >
                ...
                <Select cursor={"pointer"} placeholder="Select City" value={city} width={"20em"} onChange={(e) => handleChange(e.target.value)}>
                  {Cities.map((item, index) =>item.city ? (
                      <option key={index} style={{ cursor: "pointer" }} value={item.city}>
                        {item.city}
                      </option>
                    ) : (
